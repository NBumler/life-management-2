...
export function routeRowToDto(row: RouteRow): Route {
  return {
    ...
    aspect: (row.aspect as Route.AspectEnum | null) ?? null,
    topoNumber: row.topo_number,
    protectionType: (row.protection_type as Route.ProtectionTypeEnum | null) ?? null,
    deleted: row.deleted === 1,
    deletedAt: row.deleted_at,
    createdAt: row.created_at ?? undefined,
    updatedAt: row.updated_at ?? undefined,
...
export function routeLocalWriteTask(dto: Route): SqlTask {
  return {
    statement: `
      INSERT INTO route (id, sector_id, name, guidebook_grade, length_in_meters, total_pitches, rock_type, aspect, topo_number, protection_type, _dirty, _local_only)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 1)
      ON CONFLICT(id) DO UPDATE SET
        ...
        protection_type = excluded.protection_type, _dirty = 1`,
    values: [
      dto.id,
      dto.sectorId,
      dto.name,
...
export function routeServerApplyTask(dto: Route): SqlTask {
  return {
    statement: `
      INSERT INTO route (id, sector_id, name, guidebook_grade, length_in_meters, total_pitches, rock_type, aspect, topo_number, protection_type, created_at, updated_at, deleted, deleted_at, _dirty, _local_only)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0)
      ON CONFLICT(id) DO UPDATE SET
        sector_id = excluded.sector_id, name = excluded.name, guidebook_grade = excluded.guidebook_grade,
        length_in_meters = excluded.length_in_meters, total_pitches = excluded.total_pitches,
        rock_type = excluded.rock_type, aspect = excluded.aspect, topo_number = excluded.topo_number, protection_type = excluded.protection_type,
        created_at = excluded.created_at, updated_at = excluded.updated_at, deleted = excluded.deleted, deleted_at = excluded.deleted_at,
        _dirty = 0, _local_only = 0, _needs_refetch = 0
...
export interface ClimbingSessionRow {
  ...
  // backlog/084: `sector_id` / `sector_name` moved to `ascent_attempt`; `rock_type` / `aspect` are no
  // longer stored on a session (master-data properties, resolved client-side for display). The columns
  // still exist in the SCHEMA_V22 `climbing_session` table as dead NULL columns — never read/written.
  created_at: string | null;
  updated_at: string | null;
  deleted: number;
  deleted_at: string | null;
  ...
  return {
    statement: `
      ...
      ON CONFLICT(id) DO UPDATE SET
        ...
        route_name = excluded.route_name, length_in_meters = excluded.length_in_meters,
...
export function hikeRouteRowToDto(row: HikeRouteRow): HikeRoute {
  return {
    ...
    days: row.days ? (JSON.parse(row.days) as HikeRoute['days']) : null,
  };
}
  ...
  return {
    statement: `
      INSERT INTO hike_route (
        ...
        estimated_duration_minutes, elevation_profile, days, _dirty, _local_only)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 1)
      ON CONFLICT(id) DO UPDATE SET
        ...
        elevation_profile = excluded.elevation_profile, days = excluded.days, _dirty = 1`,
    values: [
      dto.id,
      dto.name,
      ...
      dto.days ? JSON.stringify(dto.days) : null,
    ],
  };
}
...
export function hikeRouteServerApplyTask(dto: HikeRoute): SqlTask {
  return {
    statement: `
      INSERT INTO hike_route (
        id, name, coordinates, distance_meters, elevation_gain_meters, elevation_loss_meters,
        estimated_duration_minutes, elevation_profile, days, created_at, updated_at, deleted, deleted_at, _dirty, _local_only)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0)
      ON CONFLICT(id) DO UPDATE SET
        name = excluded.name, coordinates = excluded.coordinates,
        distance_meters = excluded.distance_meters, elevation_gain_meters = excluded.elevation_gain_meters,
        elevation_loss_meters = excluded.elevation_loss_meters, estimated_duration_minutes = excluded.estimated_duration_minutes,
        elevation_profile = excluded.elevation_profile, days = excluded.days,
        created_at = excluded.created_at, updated_at = excluded.updated_at, deleted = excluded.deleted, deleted_at = excluded.deleted_at,
        _dirty = 0, _local_only = 0, _needs_refetch = 0
