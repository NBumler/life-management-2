 ...
 * indoor boulder reference (documentation/Subfeatures/Indoor boulder napló.md) by: a session-level
...
export class OutdoorBoulderSessionEditPage implements OnInit {
  ...
  /** Problems under the sector picked on this attempt row (backlog/084 — sector is per attempt). */
  problemsForRow(row: AttemptRow): BoulderProblem[] {
  ...
  problemById(id: string | null): BoulderProblem | undefined {
    return id ? this.boulderProblemRepository.items().find((problem) => problem.id === id && !problem.deleted) : undefined;
  }

  /**
   * backlog/092 — date of a prior successful ascent of this row's linked problem when the row is a
  ...
  priorAscentWarningDate(row: AttemptRow): string | null {
    ...
    return this.repository.priorSuccessfulAscentDate(row.boulderProblemId(), this.form.controls.date.value, this.sessionId());
  }
  ...
  /** backlog/084 — a picked sector snapshots its name onto the row; a problem no longer under it is cleared. */
  pickSector(row: AttemptRow, sectorId: string | null): void {
    ...
    if (row.boulderProblemId() && !this.problemsForRow(row).some((problem) => problem.id === row.boulderProblemId())) {
      row.boulderProblemId.set(null);
    }
    this.touchAttempts();
  }
   ...
   * flagged for the catalog becomes a `BoulderProblem` master under **that row's** sector (backlog/084);
   * the new id is written back onto the row so `buildDraft()` links to it. No sector → nothing to attach to.
   */
  private async persistNewCatalogProblems(): Promise<void> {
    for (const row of this.attempts()) {
      const sectorId = row.sectorId();
      if (!sectorId || !row.saveToCatalog() || row.boulderProblemId()) {
        continue;
      }
