...
- **L2 — Health Connect sync**: `core/health/` — `activity-step-sync.service`
  (app-nyitás + `App` `resume` → mai nap + 7 napos hiánypótló backfill csak a
  `DailyStepLog`-gal nem rendelkező napokra, `datesNeedingBackfill` pure modul),
  `health-connect.plugin.ts` (`registerPlugin('HealthConnectSteps')` TS-szerződés),
...
- **A natív Android Health Connect modul** — a `HealthConnectSteps` plugin csak a
  TS-oldali szerződés; a Kotlin implementáció (vagy egy Capacitor 8-kompatibilis
  közösségi csomag) még hiányzik, addig minden hívás „nincs adat"-ot ad vissza és
  csak a kézi bevitel él. Ugyanaz az állapot, ahonnan a vonalkód-scanner is indult
  („Kész, on-device ellenőrzés hátravan").
