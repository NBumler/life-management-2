...
| [Pénzügyek](documentation/Features/Pénzügyek.md) | `d1950b4` (2026-08-19) | — | — | **Folyamatban (P1 kész).** Menü → Pénzügyek hub + két gyerek, subfeature-önkénti bontásban (Rendszeres kiadások → Nettó fizetés kalkulátor → hub + `menu.penzugyek` élővé tétele). A hubnak nincs saját entitása / OpenAPI-ja; fogyasztó. **P1:** [[Rendszeres kiadások]] kész — lásd külön sor lent. Hátra: [[Nettó fizetés kalkulátor]] (P2) + `FinanceDashboardPage` hub + menüpont (P3). A `menu.penzugyek` flag már `true` a `features.json`-ban, de a menüpont + `finance` index route csak P3-ban kerül be, így a köztes commitok érvényesek. |
