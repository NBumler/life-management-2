...
| [Tennivalók](documentation/Features/Tennivalók.md) | `d1950b4` (2026-08-19) | `hu.bumler.lm2.tasks` (`LifePlan*`, `HouseholdRoom*`, `HouseholdTask*`, `CalendarEvent*`) | `pages/tasks/` (hub + life-plans/household/events/calendar) | 4/4 subfeature kész, hub + routing véglegesítve. Flag-ek (`tab.feladatok` + 3 csempe) bekapcsolva a `features.json`-ban. Az értesítések (`HOUSEHOLD_TASK_DUE`, `EVENT_OCCURRENCE`) az [[Értesítések]] körben (2026-09-01) elkészültek — a producer csak az élő `nextDue` / a vetített előfordulások, tartalmi vizsgálat nélkül. Google export továbbra sincs (flag `false`). |
...
| [Lépésszám követés](documentation/Features/Lépésszám%20követés.md) | `d1950b4` (2026-08-19) | `hu.bumler.lm2.steps` (`DailyStepLog` — Entity/Mapper/Repository/Service/Controller/SyncDataLoader, `daily_step_log` tábla V29) | `pages/menu/steps/` (`step-tracker.page` mai érték + múltbeli lista + Health Connect státusz-sáv, `step-log-edit.page` per-nap), `core/data/daily-step-log.repository.ts`, `core/data/activity-kcal.ts` `stepKcalForDay`, `core/health/` (`activity-step-sync.service` + `step-sync-plan.ts` + `health-connect(.plugin)`), `android/.../health/HealthConnectStepsPlugin.kt`, teljes offline-sync bekötés (`DailyStepLog` outbox entityType, SQLite séma v27) | **Kész (L1) + Kész (L2, on-device funkcionális próba hátravan).** Lapos user-owned CRUD `SwimLog` mintára, `id` = determinisztikus v5 `DailyStepLog:<userId>:<date>` → két offline eszköz konvergál, POST törölt napra `WeeklyPlan`-mintára revive. Szerver sima last-write-wins; a felülírási szabály (manuális mindig nyer, sync csak ha nagyobb) a `DailyStepLogRepository`-ban (`saveManual` / `maxWinsUpsert`). Parciális unique index `(user_id, log_date)` védőháló + `GlobalExceptionHandler` map. Lépéskalória SSOT: `stepKcalForDay` = `max(0, stepCount − 3000) × testsúly × 0.00045` (`STEP_BASELINE` a fix 1.2 PAL-ban), bekötve a Kaja dashboard `workoutExtraKcal`-jába. **L2 (Health Connect):** `activity-step-sync.service` app-nyitáskor + `App` `resume`-kor a mai napot + 7 napos hiánypótló backfillt húzza (`datesNeedingBackfill` pure) `maxWinsUpsert`-tel; `HealthConnectStepsPlugin.kt` app-lokális Capacitor plugin (`androidx.health.connect:connect-client:1.1.0`, `getSdkStatus` / `permissionController` / `aggregate(StepsRecord.COUNT_TOTAL)`), `MainActivity`-ben regisztrálva; Kotlin a app-modulban (`kotlinVersion` a `variables.gradle`-ben). `./gradlew :app:assembleDebug` zöld, a plugin + a HC SDK benne a debug APK-ban; a valós Samsung Health adathalmazzal való funkcionális próba on-device hátravan. AndroidManifest: `health.READ_STEPS` + `SHOW_PERMISSIONS_RATIONALE` + `VIEW_PERMISSION_USAGE` activity-alias + `<queries>` + `uses-sdk tools:overrideLibrary` (HC minSdk 26 vs app 24). **Tudatosan kihagyva:** valódi 08:00 `WorkManager` háttér-worker (nincs `background-runner`; az app-open backfill a tartalék), „Frissítés most" gomb, iOS Health. A `STEPS_LOW` értesítés (20:00, mai < 2000) az [[Értesítések]] körben (2026-09-01) elkészült. `menu.lepesszam` flag `true`. Lásd „Lezárt kör: Lépésszám követés". |
| [Értesítések](documentation/Features/Értesítések.md) | `4562923` (2026-08-19) | — (tisztán kliensoldali, nincs backend érintettség) | `core/notifications/` (`notification-scheduler.service` = `NotificationScheduler` root service, `notification-rules.ts` pure döntési réteg mind a 6 típusra, `notification-settings.service` device-local kapcsolók, `notification-dedupe.store` „már elküldve" napló, `notification-ids.ts` determinisztikus int id, `local-notifications.gateway.ts` a `@capacitor/local-notifications` Proxy-seam), `pages/menu/notifications/` (típus-kapcsolók + engedély-sáv), `notifications` route `featureFlagGuard('menu.ertesitesek')`-kel, `menu.page` Menü-pont, `main.ts` cold-start 6 + `login.page` re-invoke, AndroidManifest `POST_NOTIFICATIONS` + `SCHEDULE_EXACT_ALARM` | **Kész (egy körben).** Mind a 6 aktív típus (`FOOD_EXPIRING_DAILY`, `FOOD_SPOILED_ONCE`, `STEPS_LOW`, `CALORIE_STREAK`, `HOUSEHOLD_TASK_DUE`, `EVENT_OCCURRENCE`) él, natív-only (web = no-op, Platform-képességmátrix). Modell: minden újraértékelési triggerkor (cold start, `App` resume, forrás-signal / `DataChangeNotifier` mutáció, típus-kapcsoló, nyelvváltás) a `notification-rules` kiszámolja a kívánt halmazt egy rövid horizonton (fix-idős típusok: következő előfordulás; `EVENT_OCCURRENCE`: +30 nap), majd reconcile a `getPending()` ellen — múltbeli esedékes → azonnali fire egyszer + dedup-napló, jövőbeli → `schedule.at`, már nem kívánt (törölt tétel / teljesített feladat / 2000 fölé ment lépés 20:00 előtt) → `cancel`. Dedup-napló (`@capacitor/preferences`) állja a restart utáni újrafire-t; a kiment banner sosem vonódik vissza. A típus-kapcsolók **nem** feature flag-ek (device-local, nem syncelnek); a forrás-feature flagje fedi őket. Nyelvváltáskor a registry-be sütött `lang` alapján cancel+újraütemez, a dedup-napló nem ürül. `menu.ertesitesek` flag `true`. **Tudatosan kihagyva:** valódi 08:00/20:00 háttér-worker (nincs `@capacitor/background-runner`) — ha az app egy nap meg sem nyílik, a fix-idős értesítés kimaradhat, a következő app-open azonnal pótolja ha még releváns; **ez a következő javasolt feature, lásd lent.** Nincs lead-time szerkesztő, nincs értesítés-előzmény lista, nincs remote push. Lásd „Lezárt kör: Értesítések". |
...
(2026-09-01, lásd „Lezárt kör: Értesítések"). Ami hátravan, az már nem önálló
feature, hanem megerősítés / kiegészítés:

- **08:00/20:00 háttér-értesítés worker** — a következő javasolt feladat, lásd lent.
- **On-device funkcionális próbák** — Health Connect lépés-sync valós Samsung Health
  adattal; helyi értesítések valós telefonon (engedélykérés, 09:00/20:00 fire,
  tap → route).
...
- **Lead-time szerkesztő** (a szabályok fixek a specben), **értesítés-előzmény
  lista**, **remote push (FCM/APNs)** — a spec is mind későbbi scope-nak jelöli.
...
## Következő javasolt feladat: **08:00 / 20:00 háttér-értesítés worker**

Az [[Értesítések]] és a [[Lépésszám követés]] L2 kör is ugyanazt hagyta nyitva: a
`@capacitor/background-runner` (vagy natív `WorkManager` / `AlarmManager` modul) még
nincs bekötve, ezért minden ütemezett újraértékelés app-nyitáshoz / `resume`-hoz
kötött. Egy napi 1–2 alkalommal futó háttérfeladat, ami

1. a Health Connect mai + backfill syncet lefuttatja ([[Lépésszám átszinkronizálása
   a Samsung Health-ből]] 08:00 worker terve), és
2. meghívja a `NotificationScheduler.reevaluate('background')`-ot,

garantálná, hogy a `STEPS_LOW` 20:00-kor és a 09:00-ás digestek akkor is
kimenjenek, ha a user aznap nem nyitotta meg az appot. A `NotificationScheduler`
API-ja (`reevaluate`) és a pure `notification-rules` már készen áll rá; a munka a
háttér-JS-kontextus + DI-mentes belépő megépítése.
