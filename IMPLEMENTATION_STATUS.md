...
| [Bevásárlás](documentation/Features/Bevásárlás.md) | `3ddf321` (2026-08-14) | — | — | Csak a hub navigáció (Menü tab, nem Kaja alatt); a három subfeature közül egy (Bevásárlólista írás) kész, a másik kettő (Bevásárlás teljesítve, Bevásárlás előzmény) hátravan — lásd alul. |
| ↳ [Bevásárlólista írás](documentation/Subfeatures/Bevásárlólista%20írás.md) | `3ddf321` (2026-08-14) | `hu.bumler.lm2.food` (`ShoppingList`, `ShoppingListItem` — per-user, `ShoppingListItem` polimorf FOOD/NON_FOOD, kolokálva a `food` csomagban a `Food` FK-referencia miatt, ugyanúgy mint `Meal`/`MealItem`) | `pages/menu/shopping/` (lista + szerkesztő: névmező, FOOD-picker a meal-edit mintájára, NON_FOOD inline sor név+mennyiség+szabad szöveggel, pipa checkbox), `core/data/shopping-list.repository.ts` | **Kész.** Nested aggregate PUT (mint `Meal`/`MealItem`), de a Meal-lel ellentétben **nincs "legalább egy tétel" megkötés** — a spec explicit megengedi az üres aktív listát (kézi törléssel, nem automatikus cascade-del). `status`/`completedAt` mezők megvannak a sémában, de ebben a körben read-only-k (csak a jövőbeli `Bevásárlás teljesítve` `POST .../complete` végpont fogja állítani). Food törléskor cascade a rá hivatkozó ShoppingListItem sorokra (`ShoppingListItemCascade`, a `MealCascade`-től eltérően **nem** töröl automatikusan üresen maradt listát), mindkét oldalon (backend `FoodService.delete`, frontend `SqliteStorageBackend`/`SyncEngine` tombstone/pull ágak). A "Bevásárlás vége" belépő tudatosan **nincs** ebben a körben — a még nem létező [[Bevásárlás teljesítve]] flow-ra mutatna. `menu.bevasarlas` feature flag bekapcsolva. |
...
| ↳ Bevásárlás folytatása | Bevásárlás teljesítve → Bevásárlás előzmény (Bevásárlólista írás már kész — lásd "Folyamatban" fent) | Élelmiszer tárolás (kész, a teljesítve flow ebbe ír) |
