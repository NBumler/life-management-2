...
| [Edzés](documentation/Features/Edzés.md) | `61daa22` (2026-08-19) | — (közös edzés-domain a gyerekekben) | `pages/workout/` (tab shell + `workout-segment-header.component.ts` megosztott felső szegmens + `workout-sections.ts` registry), `app.routes.ts` `/tabs/workout` fa, `core/data/activity-kcal.ts` | **Folyamatban — Edzésnapló + Heti terv kész és élő (A0–A4).** Edzésnapló \| Heti terv \| Mászás \| Úszás \| Bicikli felső szegmens; a `log` és a `weekly-plan` route be van kötve, a `maszas`/`uszas`/`bicikli` szegmensek route-jai a saját slice-ukkal jönnek (`edzes.maszonaplo`/`edzes.uszas`/`edzes.bicikli` flag még `false`). **A4 (ebben a commitban):** `tab.edzes` + `edzes.hetiTerv` → `true` a `features.json`-ban → az Edzés tab + a Heti terv szegmens látható; `activity-kcal.ts` (`workoutKcalForDay` — Σ `sessionKcal()` az aznapi élő session-ökre a Profile testsúlyával) bekötve a Kaja dashboard `computeTdee` 3. argumentumába (eddig fix 0). Lépéskalória továbbra is 0 ([[Lépésszám követés]] nincs kész). Hátra: Mászónapló, Úszás/Biciklizés napló. |
...
a tab-váz + Gyakorlat + Edzésnapló + Heti terv + aktiválás (A0–A4) kész és élő;
hátra: Mászónapló, Úszás/Biciklizés napló.)
...
- **A4 — aktiválás + activityExtraKcal** (ebben a commitban): `tab.edzes` + `edzes.hetiTerv`
