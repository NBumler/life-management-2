...
| [Edzés](documentation/Features/Edzés.md) | `61daa22` (2026-08-19) | `hu.bumler.lm2.workout` (a gyerekek entitásai; közös edzés-domain) | `pages/workout/` (tab shell + `workout-segment-header.component.ts` megosztott felső szegmens + `workout-sections.ts` registry), `app.routes.ts` `/tabs/workout` fa, `core/data/activity-kcal.ts` | **Folyamatban — Edzésnapló + Heti terv + Úszás napló kész és élő (A0–A5).** Edzésnapló \| Heti terv \| Mászás \| Úszás \| Bicikli felső szegmens; a `log`, a `weekly-plan` és a `swimming` route be van kötve, a `maszas`/`bicikli` szegmensek route-jai a saját slice-ukkal jönnek (`edzes.maszonaplo`/`edzes.bicikli` flag még `false`). **A4:** `tab.edzes` + `edzes.hetiTerv` → `true`; `activity-kcal.ts` `workoutKcalForDay` bekötve a Kaja dashboard `computeTdee` 3. argumentumába. **A5 (ebben a commitban):** [[Úszás napló]] — lásd külön sor lent; `edzes.uszas` → `true`, `swimKcalForDay` hozzáadva a Kaja dashboard aktivitás-kalóriájához. Lépéskalória továbbra is 0 ([[Lépésszám követés]] nincs kész). Hátra: Mászónapló, Biciklizés napló. |
...
a tab-váz + Gyakorlat + Edzésnapló + Heti terv + aktiválás + Úszás napló (A0–A5)
kész és élő; hátra: Mászónapló, Biciklizés napló.)
...
Hátra: külön körre a **Mászónapló** ág (egyedül
12 al-spec, Indoor/Outdoor × boulder/köteles + nehézségi konverziós mátrix) és a
**Biciklizés napló** (az Úszás napló mintáját tükrözi).
