...
Hátra a Mászónapló-alkörből: M1 (hub váz + `climbing` szegmens route), M2a/M2b
(backend törzsadat + session nested aggregate), M3 (admin frontend), M4–M7 (a 4
kontextus-napló), M8 (mászó statisztikák + `edzes.maszonaplo` aktiválás +
`climbingKcalForDay` a Kaja dashboardba).
