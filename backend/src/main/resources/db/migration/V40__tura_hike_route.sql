-- backlog/tura-utvonaltervezo/103-... 2.1 fázis: kézzel rajzolt túraútvonal, user-owned, teljesen
-- offline-szinkronizált entitás (ellentétben a trail_segment-tel, ami megosztott/nem szinkronizált
-- referenciaadat — lásd V39). coordinates lapított [lon1, lat1, lon2, lat2, ...] tömb, a
-- trail_segment mintáját követve (natív Postgres double precision[]); legalább 2 pont (4 elem)
-- kell egy vonalhoz.

CREATE TABLE hike_route (
    id          uuid PRIMARY KEY,
    user_id     uuid NOT NULL REFERENCES users (id),
    name        text NOT NULL,
    coordinates double precision[] NOT NULL
                CHECK (array_length(coordinates, 1) >= 4 AND array_length(coordinates, 1) % 2 = 0),
...
CREATE INDEX idx_hike_route_user_id_updated_at ON hike_route (user_id, updated_at);

CREATE TRIGGER hike_route_set_updated_at
    BEFORE INSERT OR UPDATE ON hike_route
    FOR EACH ROW
...
-- trail_segment is deliberately NOT included (shared/global reference data, bbox-queried, not synced
-- — backlog/tura-utvonaltervezo/103-... 1. fázis).
CREATE OR REPLACE VIEW sync_changes AS
    ...
    UNION ALL
    SELECT 'HikeRoute' AS entity_type, id, user_id, updated_at, deleted FROM hike_route;
