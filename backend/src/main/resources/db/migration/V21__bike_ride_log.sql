-- documentation/Features/Biciklizés napló.md — one logged bike ride ("egy napló = egy út"), under
...
-- canonical MET formula (documentation/Features/Tápérték kalkulátor.md). distance_km /
-- elevation_gain_meters are log/statistics only (plus a UI avg-speed MET hint). Mirrors swim_log
-- (V20) — no pool-pairing rule here, the two optional fields are independent.

CREATE TABLE bike_ride_log (
    id                    uuid PRIMARY KEY,
    ...
    intensity             text NOT NULL CHECK (intensity IN (
                              'CITY', 'STATIONARY', 'ROAD_LEISURE', 'MOUNTAIN_TRAIL', 'ROAD_VIGOROUS')),
    -- optional; log / statistics + UI avg-speed hint only. Never in the kcal formula.
...
CREATE INDEX idx_bike_ride_log_user_id_updated_at ON bike_ride_log (user_id, updated_at);
-- documentation/Features/Biciklizés napló.md "Lista": aznapi / időrendi lookups by date.
CREATE INDEX idx_bike_ride_log_user_id_ride_date ON bike_ride_log (user_id, ride_date);

CREATE TRIGGER bike_ride_log_set_updated_at
    BEFORE INSERT OR UPDATE ON bike_ride_log
    FOR EACH ROW
