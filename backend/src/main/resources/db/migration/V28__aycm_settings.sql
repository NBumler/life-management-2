-- documentation/Features/AYCM tracker.md — the 1:1-per-user AYCM settings singleton. Holds only the
-- optional link to a RecurringExpense (the monthly pass). There is NO FK to recurring_expense: the
-- coupling is loose (documentation says "a kliens ellenőrzi a beszámítást"), so a linked expense
-- that is later deleted or paused simply stops counting on the client. id is a deterministic client
-- UUID v5 of "AycmSettings:<userId>" (Backend-offline first.md §9), so two offline devices converge.

CREATE TABLE aycm_settings (
    id                          uuid PRIMARY KEY,
    user_id                     uuid NOT NULL REFERENCES users (id),
    linked_recurring_expense_id uuid,
...
-- 1:1 per user (documentation/Architektúra/Backend-offline first.md §9: the deterministic v5 id
-- guarantees this on the client side too).
CREATE UNIQUE INDEX idx_aycm_settings_user_id ON aycm_settings (user_id);

CREATE TRIGGER aycm_settings_set_updated_at
    BEFORE INSERT OR UPDATE ON aycm_settings
    FOR EACH ROW
