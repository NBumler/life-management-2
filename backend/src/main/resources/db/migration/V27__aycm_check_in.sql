-- documentation/Subfeatures/AYCM Check-In.md — one AYCM entry per calendar day. Flat, user-owned.
-- The row carries a full snapshot taken at save time (partner name, band label, list price, co-pay,
-- visit value); the server never re-runs matchPriceRule, it just stores what the client sent.
-- visitValueHuf = listPriceHuf; coPaymentHuf is metadata and never feeds the visit value.
-- check_in_time is 'HH:mm' (minute 0..1439, never '24:00'). Past, today and future dates are all
-- allowed. partner_id has NO FK to a live partner — a deleted partner's uuid stays for statistics.

CREATE TABLE aycm_check_in (
    id             uuid PRIMARY KEY,
    ...
    co_payment_huf integer NOT NULL CHECK (co_payment_huf >= 0),
    visit_value_huf integer NOT NULL CHECK (visit_value_huf >= 0),
    notes          text,
...
-- documentation/Subfeatures/AYCM Check-In.md "Egyediség": max 1 live Check-In per user per calendar day.
CREATE UNIQUE INDEX idx_aycm_check_in_user_id_check_in_date
    ON aycm_check_in (user_id, check_in_date) WHERE deleted = false;

-- documentation/Architektúra/Backend.md "Indexek": delta pull filter.
CREATE INDEX idx_aycm_check_in_user_id_updated_at ON aycm_check_in (user_id, updated_at);

CREATE TRIGGER aycm_check_in_set_updated_at
    BEFORE INSERT OR UPDATE ON aycm_check_in
    FOR EACH ROW
