...
- **Friss lépésszám a küszöb előtt:** a `NotificationScheduler` minden refresh-ágú újraértékelés (app-nyitás / előtérbe jövés / reconcile) előtt — ha a `STEPS_LOW` aktív — meghívja az `ActivityStepSyncService.syncTodayForNotification()`-t: **csak a mai nap** friss Health Connect olvasása + max-wins upsert (backfill és `lastSyncAt` nélkül), és a küszöböt a frissített helyi értékből dönti el. Így egy reggel óta nem syncelt `DailyStepLog` nem küld valótlan „kevés lépés" értesítést. Foreground `READ_STEPS` grant nélkül / weben no-op → marad az utolsó ismert helyi érték. A natív háttér-worker esti ága (ha van `READ_HEALTH_DATA_IN_BACKGROUND` grant) már ma is élő Health Connect olvasásból dönt; enélkül nem tud friss értéket olvasni (lásd „Tudatos korlát").
...
- **Státuszsor-ikon:** minden banner a `ic_stat_notify` monokróm sziluettet viszi (az app „egymásra csúszó lapok" jele, fehér-átlátszó — az OS maga színezi), `#3880ff` akcentussal. Nem az app launcher-ikonja (az fehér négyzetként jelenne meg a státuszsorban).
...
- **`STEPS_LOW` háttér-worker friss olvasás nélkül:** `READ_HEALTH_DATA_IN_BACKGROUND` grant hiányában a natív háttér-worker a zárt app mellett nem tud friss Health Connect olvasást végezni a 20:00-s `STEPS_LOW` előtt — ilyenkor az utolsó ismert helyi `DailyStepLog` értékből tüzel, ami elavult lehet. A foreground / app-nyitáskori út a küszöb előtt mindig friss olvasást végez (`backlog/081`); a háttér-worker engedély nélküli ága marad ez a kompromisszum.
...
- A `STEPS_LOW` esti Health Connect olvasásához az `android.permission.health.READ_HEALTH_DATA_IN_BACKGROUND` engedély kell (a Lépésszám képernyőn külön „háttér-hozzáférés" prompt, csak a foreground `READ_STEPS` után kérhető). Megtagadva a `STEPS_LOW` marad app-nyitáskor értékelődő — de az app-nyitáskori / előtérbe-jövési refresh-reconcile a küszöb kiértékelése előtt maga is friss **foreground** Health Connect olvasást futtat a mai napra (`ActivityStepSyncService.syncTodayForNotification()` — csak ma, backfill nélkül; ha épp fut egy teljes app-nyitáskori sync, azt megvárja), így az elavult `DailyStepLog` nem okoz valótlan `STEPS_LOW`-t (`backlog/081`). A háttér-worker engedély nélküli ága ezt nem tudja pótolni (lásd „Tudatos korlát"). Az agresszív OEM-akkumulátor-optimalizálás (Samsung/Xiaomi) a riasztást órákkal késleltetheti vagy kilőheti — a végső védőháló továbbra is az app-open reconcile.
...
- **Kis (státuszsor-) ikon:** a Capacitor-ütemezett bannerek a `plugins.LocalNotifications.smallIcon = 'ic_stat_notify'` + `iconColor = '#3880ff'` configból (`capacitor.config.ts`) kapják az ikont/színt; a natív `ReminderWorker` ugyanezt kézzel állítja (`setSmallIcon(R.drawable.ic_stat_notify)` + `setColor(R.color.notification_accent)`), mert a `NotificationCompat.Builder`-t közvetlenül hívja. A `res/drawable-*/ic_stat_notify.png`-t az `assets/notification-icon.svg`-ből a `npm run gen:assets` rendereli (a launcher-ikonnal és a splash-sel együtt).
